...
def load_DF (sensorVsCycle,year,month,day):
  url = "http://172.20.0.1:7770/"+sensorVsCycle+"/fetch_data"
  RawDataRequest = {
    'year': year,
    'month': month,
    'day': day
...
def cycle_train():
  ...
  url = "http://172.20.0.1:7770/Cycle/fetch_data"
  days,cycles,X,Y = [],[], [],[] 
  window_size = 3600
...
def cycle_detector(year,month, day , Resolution, window_size):
  ...
  df1["Cycle"] = pd.to_numeric(df1["Cycle"], downcast="float")
...
@app.route('/cycle_checker',methods = ['GET'])
def cycle_checker():
  cycles = cycle_detector(request.json['year'],request.json['month'],request.json['day'], 50, window_size)
  return jsonify({"cycles":cycles.tolist()})

@app.route('/anomaly_checker',methods = ['GET'])
def anomaly_checker():
  Feed,normality = [],[]
  cycles_of_day = cycle_detector(request.json['year'],request.json['month'],request.json['day'] , 20, window_size)
  df2 = load_DF("Sensor",request.json['year'],request.json['month'],request.json['day'])
  for cycle in cycles_of_day:
    f = np.array(df2.iloc[cycle:cycle+window_size-2*margin]['sensor']).reshape(-1,1)
    Feed.append (f)
  day = np.array(Feed)
  model_m = load_model('model/DummyAnomaly.h5')
  daily_Predict = model.predict(day).reshape(len(cycles_of_day),window_size-2*margin,1)
  thresh = np.mean(np.abs(daily_Predict - day), axis=1)
  ...
  for item in thresh:
    if (1/item > 125):
      ...
      normality.append(0)
  return jsonify({"cycles": cycles_of_day.tolist(),"anomaly": normality.tolist()})
